...
import {isServerEvent, ServerEventPayloadMap, serverEventSchemaMap, ServerEventType} from "./serverEventDefinitions";
...
export class EventSocketService {
    ...
    on<T extends ServerEventType>(event: ServerEventType, handler: (payload: ServerEventPayloadMap[T]) => void): () => void {
    ...
    private catchallHandler(event: ServerEventType, payload: Record<string, unknown>) {
        ...
        const validationResult = serverEventSchemaMap[event].safeParse(event)
